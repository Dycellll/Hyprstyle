...
uninstall() {
    ...
    if [ -d "$CONFIG_DIR" ]; then
        printf "Keep your styles and config at %s? [Y/n] " "$CONFIG_DIR"
        read -r answer
        case "$answer" in
            [Nn]*)
                rm -rf "$CONFIG_DIR"
...
printf "Install hyprstyle to %s? [y/N] " "$INSTALL_DIR"
read -r answer
case "$answer" in
    [Yy]*) ;;
    *) echo "Aborted."; exit 0 ;;
...
ARCH=$(uname -m)
case "$ARCH" in
    x86_64) ASSET="hyprstyle-linux-x64" ;;
    aarch64|arm64) ASSET="hyprstyle-linux-arm64" ;;
